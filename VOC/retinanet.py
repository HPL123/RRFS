...
#
#
#     outputs = keras.layers.Conv2D( # Shafin
#         filters=300, ################################################
#         activation='relu',
#         name='pyramid_classification_shafin',
#         kernel_initializer=keras.initializers.normal(mean=0.0, stddev=0.01, seed=None),
#         bias_initializer='zeros',
#         **options
#     )(outputs)
#
#     def my_init(shape, dtype=None): # Shafin
#         # vec = np.random.randn(300, 80)
#         vec = np.loadtxt('Matlab/word_glo_.txt', dtype='float32', delimiter=',') ##########################################
#         vec = vec[:,:177]
#
#         # vec2 = np.repeat(vec, 9, axis=1)
#         # vec3 = np.expand_dims(np.expand_dims(vec2, axis=0), axis=0)
#         # vec4 = np.repeat(np.repeat(vec3, 3, axis=0), 3, axis=1)
#         # weight = K.variable(vec4,dtype='float32')
#
#         v2 = np.repeat(vec, 9, axis=0)
#         v3 = np.reshape(v2, (vec.shape[0], vec.shape[01] * 9))
#         vec3 = np.expand_dims(np.expand_dims(v3, axis=0), axis=0)
#         vec4 = np.repeat(np.repeat(vec3, 3, axis=0), 3, axis=1)
#         weight = K.variable(vec4, dtype='float32')
#         return weight
#
...
#         kernel_initializer=my_init, #keras.initializers.zeros(),
#         trainable=False, #shafin
#         bias_initializer=initializers.PriorProbability(probability=prior_probability),
#         name='pyramid_classification',
#         **options
#     )(outputs)
...
#     # outputs = keras.layers.Activation('sigmoid', name='pyramid_classification_sigmoid')(outputs) # sigmoid linear
#
#     return keras.models.Model(inputs=inputs, outputs=outputs, name=name)
#
#
...
#     # additionally apply non maximum suppression
#     if nms:
#         detections = layers.NonMaximumSuppression(name='nms', score_threshold=1e-4)([boxes, classification] + other) # score_threshold=0.05
#     else:
#         detections = keras.layers.Concatenate(axis=2, name='detections')([boxes, classification] + other)
#
#     # construct list of outputs
